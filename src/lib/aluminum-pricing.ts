export type AluminumMaterial = "040" | "080";
export type AluminumPrintMode = "single" | "double";
export type AluminumPricingMode = "sheet" | "sqin";

// ─── Size options ────────────────────────────────────────────────────────────

export interface AluminumSizeOption {
  id: string;
  width: number;
  height: number;
}

export const ALUMINUM_SHEET = { width: 48, height: 96 };
...
export const ALUMINUM_SIZE_OPTIONS: AluminumSizeOption[] = [
...
export function formatAluminumSize(size: AluminumSizeOption): string {
  return `${size.width}" x ${size.height}"`;
}

// ─── Sheet layout (same algorithm as other rigid builders) ───────────────────

export interface AluminumSheetPlacement {
  x: number;
  y: number;
  width: number;
  height: number;
...
export interface AluminumSheetLayout {
  count: number;
  placements: AluminumSheetPlacement[];
}

function buildGrid(
  ox: number, oy: number, rw: number, rh: number,
  sw: number, sh: number, rotated: boolean
): AluminumSheetPlacement[] {
  const cols = Math.floor(rw / sw);
  const rows = Math.floor(rh / sh);
  const placements: AluminumSheetPlacement[] = [];
...
function pick(a: AluminumSheetLayout, b: AluminumSheetLayout): AluminumSheetLayout {
  return b.count > a.count ? b : a;
}

function sizeKey(w: number, h: number): string { return `${Number(w)}x${Number(h)}`; }

function centerPlacements(ps: AluminumSheetPlacement[]): AluminumSheetPlacement[] {
  ...
  const ox = (ALUMINUM_SHEET.width - (maxX - minX)) / 2 - minX;
  const oy = (ALUMINUM_SHEET.height - (maxY - minY)) / 2 - minY;
  return ps.map(p => ({ ...p, x: Number((p.x + ox).toFixed(4)), y: Number((p.y + oy).toFixed(4)) }));
}

function fixedLayout(w: number, h: number): AluminumSheetLayout | null {
  if (sizeKey(w, h) === "18x36") {
    const ps: AluminumSheetPlacement[] = [
...
export function getBestAluminumSheetLayout(w: number, h: number): AluminumSheetLayout {
  const fixed = fixedLayout(w, h);
  if (fixed) return fixed;

  const W = ALUMINUM_SHEET.width, H = ALUMINUM_SHEET.height;
  const n = buildGrid(0, 0, W, H, w, h, false);
  const r = buildGrid(0, 0, W, H, h, w, true);
  let best: AluminumSheetLayout = n.length >= r.length
    ? { count: n.length, placements: n }
    : { count: r.length, placements: r };
  const all: AluminumSheetLayout[] = [{ count: n.length, placements: n }, { count: r.length, placements: r }];
...
export function getAluminumSheetPrice(
  ...
  printMode: AluminumPrintMode
): number {
  const tier = SHEET_TIERS.find(t => Math.max(1, qty) <= t.maxQty) ?? SHEET_TIERS[SHEET_TIERS.length - 1];
  if (material === "040") return printMode === "single" ? tier.single040 : tier.double040;
  return printMode === "single" ? tier.single080 : tier.double080;
}
...
const SQIN_RATES: Record<string, SqinRate> = {
  "040_single": { rate: 0.09, min: 12 },
  "040_double": { rate: 0.11, min: 15 },
  "080_single": { rate: 0.14, min: 18 },
  "080_double": { rate: 0.16, min: 20 },
};

export function getAluminumSqinRate(
  material: AluminumMaterial,
  printMode: AluminumPrintMode
): SqinRate {
...
export interface AluminumPricingResult {
  mode: AluminumPricingMode;
  // Sheet mode
...
export interface AluminumSheetPricingInput {
  width: number;
  height: number;
  quantity: number;
  material: AluminumMaterial;
...
export function calculateAluminumSheetPricing(input: AluminumSheetPricingInput): AluminumPricingResult {
...
export interface AluminumSqinPricingInput {
  customWidth: number;
  customHeight: number;
  quantity: number;
  material: AluminumMaterial;
...
export function calculateAluminumSqinPricing(input: AluminumSqinPricingInput): AluminumPricingResult {
  const qty = Math.max(1, Math.floor(input.quantity));
