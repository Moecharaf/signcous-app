...
import {
  FOOTPRINTS_MIN,
  FOOTPRINTS_RATE,
  calculateFootprintsPrice,
  getFootprintsPanelInfo,
...
/** Convert a raw input string + unit to inches */
function toInches(valueStr: string, unit: DimensionUnit): number {
  const n = parseFloat(valueStr) || 0;
  return unit === "feet" ? n * 12 : n;
}

export default function FootprintsBuilder({ productId = 0 }: FootprintsBuilderProps) {
  const cart = useCart();
  ...
  const pricing = useMemo(
    () =>
      isValid
        ? calculateFootprintsPrice(widthIn, heightIn, { contourCut, rush }, safeQuantity)
        : null,
    [widthIn, heightIn, contourCut, rush, safeQuantity, isValid]
  );

  const panelInfo = useMemo(
    () => (isValid ? getFootprintsPanelInfo(widthIn, heightIn) : null),
  ...
  function addToCart() {
    ...
    cart.addItem({
      productId,
      productName: "Footprints (Floor Graphics)",
      width: widthIn,
      height: heightIn,
      unit: "inches",
      quantity: safeQuantity,
      material: "Floor Graphic",
      doubleSided: false,
      grommets: false,
      edgeFinish: "none",
      polePockets: false,
  ...
  // Preview sizing — fit within canvas while preserving aspect ratio
  ...
  // Panel grid lines for preview — percentage-based like GF2030 so lines always divide equally
    ...
    <div className="flex h-[calc(100vh-88px)] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-4">
          ...
          <div className="grid items-end gap-4 lg:grid-cols-[1fr_auto]">
            <div>
              <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-2 text-xs text-zinc-500">
                ...
                <span>/</span>
                <span className="font-semibold text-zinc-900">Footprints</span>
              </nav>
              ...
              <p className="mt-1 text-sm text-zinc-600">
                Floor graphic adhesive panels with flat-rate sq ft pricing. Dimensions are billed in
                whole feet — each dimension is rounded up before area is computed.
              </p>
            </div>
        ...
        {isValid && pricing && (
          <div className="mb-3 flex flex-wrap items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-xs font-medium text-blue-700">
            <span>
              Size:{" "}
              <span className="font-semibold">{widthIn}&quot; × {heightIn}&quot;</span>
            </span>
            <span className="text-blue-400">→</span>
            <span>
              Billable Size:{" "}
              <span className="font-semibold">
                {pricing.widthFt} ft × {pricing.heightFt} ft
              </span>
            </span>
            <span className="text-blue-400">→</span>
            <span>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
                ...
                {isValid ? (
                  ...
                  <div className="flex h-[250px] w-full max-w-[520px] flex-col items-center justify-center rounded-[28px] border border-dashed border-zinc-300 bg-white/80 px-8 text-center shadow-inner">
                    ...
                    <div className="mt-3 max-w-[30rem] text-sm leading-6 text-zinc-500">
                      Dimensions are billed to the nearest whole foot (rounded up). Panel split lines
                      shown for production guidance only — they do not affect pricing.
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="grid gap-2 border-t border-zinc-200 bg-zinc-50 p-3 md:grid-cols-6 xl:grid-cols-12">
              ...
              <ControlBox
                ...
                helper="Input value in selected unit."
              >
      ...
      <div className="flex w-[400px] flex-shrink-0 flex-col border-l border-zinc-200 bg-white">
        <div className="border-b border-zinc-100 px-5 py-4">
          ...
          <div className="mt-1 text-lg font-bold tracking-tight text-zinc-900">Footprints</div>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          <aside className="space-y-4">
            <PanelCard eyebrow="Pricing" title="Footprints Breakdown">
              <div className="space-y-1">
                ...
                <BreakdownRow
                  label="Base rate"
                  value={`${formatCurrency(FOOTPRINTS_RATE)} / sq ft`}
                  muted={!pricing}
                />
                <BreakdownRow
                  label="Base price"
                ...
                <BreakdownRow
                  label="Minimum floor"
                  value={formatCurrency(FOOTPRINTS_MIN)}
                  muted={!pricing}
                />
            ...
            <PanelCard eyebrow="Pricing Note" title="Rounding Policy">
              <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3 text-sm leading-6 text-zinc-600">
                Each dimension is rounded <strong>up</strong> to the nearest whole foot before the area
                is computed. Example: 100&quot; × 100&quot; bills as 9 ft × 9 ft = 81 sq ft.
              </div>
            </PanelCard>

            <PanelCard eyebrow="Split Note" title="Panel Split Guidance">
              <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3 text-sm leading-6 text-zinc-600">
                Panel split lines shown at 48&quot; intervals are for print-file production guidance
                only. The split selection and position offset controls do not affect pricing.
              </div>
            </PanelCard>
          </aside>
        </div>
