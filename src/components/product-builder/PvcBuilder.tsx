      ...
      <div className="w-full px-3 py-3 md:px-4">
        <div className="mb-3 grid gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm md:grid-cols-[1fr_auto] md:items-end">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-semibold tracking-tight text-zinc-900">PVC Configurator</h1>
          ...
          <div className="rounded-xl border border-[var(--brand-primary)] bg-gradient-to-r from-[var(--brand-primary-soft)] to-[#f3f8ff] px-4 py-2 text-right">
            ...
            <div className="text-xs text-[var(--brand-primary)]/80">{quantity} sign{quantity !== 1 ? "s" : ""} · {pricing.sheetsRequired} sheet{pricing.sheetsRequired !== 1 ? "s" : ""}</div>
          </div>
        </div>
          ...
          <section className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            <div className="border-b border-zinc-200 px-4 py-3">
              <div className="text-sm font-medium text-zinc-700">
                Sheet #1 / {PVC_SHEET.width}&quot; × {PVC_SHEET.height}&quot; / Front Side — {sheetLayout.count} signs per sheet
              </div>
            </div>
            ...
            <div className="grid gap-2 border-t border-zinc-200 bg-zinc-50 p-3 md:grid-cols-4 xl:grid-cols-9">
              ...
              <ControlBox title="Material">
                <select
                  value={material}
                  onChange={(event) => setMaterial(event.target.value as PvcMaterial)}
                  className="h-9 w-full rounded border border-zinc-300 bg-white px-2 text-sm"
                >
              ...
              <ControlBox title="Print Sides">
                <select
                  value={printMode}
                  onChange={(event) => setPrintMode(event.target.value as PvcPrintMode)}
                  className="h-9 w-full rounded border border-zinc-300 bg-white px-2 text-sm"
                >
