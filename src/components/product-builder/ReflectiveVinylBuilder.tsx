...
export default function ReflectiveVinylBuilder({ productId = 0 }: ReflectiveVinylBuilderProps) {
  ...
  async function onUploadContourFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setContourAlignmentConfirmed(false);
      const blobUrl = URL.createObjectURL(file);
      setContourImage((previous) => { if (previous) URL.revokeObjectURL(previous); return blobUrl; });
      if (mismatchMessage) {
  ...
  function clearContourFile() {
    ...
    setContourFileName(null);
    setContourAlignmentConfirmed(false);
    setContourImage((previous) => { if (previous) URL.revokeObjectURL(previous); return null; });
    setContourValidationError(null);
    setUploadError(null);
  }
        ...
        <div className="grid gap-4">
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            ...
            <AdhesivePricingModal
              ...
              config={ADHESIVE_PRICING_CONFIGS.reflectiveVinyl}
            />
                ...
                {isValid ? (
                  <>
                    ...
                    >
                      ...
                      {uploadedImage ? (
                        <Image src={uploadedImage} alt="Uploaded reflective vinyl artwork preview" fill unoptimized className={imageDisplayMode === "stretch" ? "object-fill" : "object-contain"} />
