...
interface ReflectiveVinylBuilderProps {
  productId?: number;
}
...
function ControlBox({
  title, helper, className, children,
}: { title: string; helper?: string; className?: string; children: ReactNode }) {
  return (
...
function PanelCard({ eyebrow, title, children }: { eyebrow: string; title: string; children: ReactNode }) {
  return (
...
function BreakdownRow({ label, value, strong, accent, muted }: {
  label: string; value: string; strong?: boolean; accent?: boolean; muted?: boolean;
}) {
  return (
    <div className="flex items-center justify-between gap-4 py-1.5 text-sm">
      <span className={muted ? "text-zinc-400" : strong ? "font-semibold text-zinc-900" : "text-zinc-600"}>{label}</span>
      <span className={`tabular-nums ${muted ? "text-zinc-400" : accent ? "font-semibold text-[var(--brand-primary)]" : strong ? "font-semibold text-zinc-900" : "text-zinc-700"}`}>
        {value}
      </span>
    </div>
  );
...
export default function ReflectiveVinylBuilder({ productId = 0 }: ReflectiveVinylBuilderProps) {
  ...
  const panelInfo = useMemo(
    () => (isValid ? getReflectiveVinylPanelInfo(widthIn, heightIn) : null),
  ...
  async function onUploadArtwork(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (file.type.startsWith("image/")) {
        ...
        setUploadedImage((previous) => { if (previous) URL.revokeObjectURL(previous); return blobUrl; });
      } else {
        setUploadedImage((previous) => { if (previous) URL.revokeObjectURL(previous); return null; });
      }
    } catch {
  ...
  const splitCount = panelInfo
    ? splitDirection === "vertical" ? panelInfo.panelsWide - 1 : panelInfo.panelsHigh - 1
    : 0;

  const positionDisplay = useMemo(() => {
    if (!panelInfo) return "—";
    const panelDimIn = splitDirection === "vertical"
    ...
    return `${(idx * panelDimIn + (splitOffsets[idx] ?? 0)).toFixed(2)}in`;
  ...
  function addToCart() {
    ...
    cart.addItem({
      productId,
      productName: "Reflective Vinyl (High-Visibility Graphics)",
      width: widthIn,
      height: heightIn,
      unit: "inches",
  ...
  const preview = useMemo(() => {
    ...
    const maxW = 620, maxH = 440, minW = 220, minH = 180;
    const fitScale = Math.min(maxW / widthIn, maxH / heightIn);
    let pw = widthIn * fitScale, ph = heightIn * fitScale;
    if (pw < minW || ph < minH) {
      const boost = Math.max(minW / pw, minH / ph);
      const bw = pw * boost, bh = ph * boost;
      if (bw <= maxW && bh <= maxH) { pw = bw; ph = bh; }
    }
    ...
    <div className="flex h-[calc(100vh-88px)] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-4">
        ...
        {isValid && pricing && (
          <div className="mb-3 flex flex-wrap items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-xs font-medium text-blue-700">
            <span>Size: <span className="font-semibold">{widthIn}&quot; × {heightIn}&quot;</span></span>
            <span className="text-blue-400">→</span>
            <span>Billable Size: <span className="font-semibold">{pricing.widthFt} ft × {pricing.heightFt} ft</span></span>
            <span className="text-blue-400">→</span>
            <span>Total Area: <span className="font-semibold">{pricing.sqFt} sq ft</span></span>
            {splitCount > 0 && (
              <>
                <span className="text-blue-400">·</span>
                <span>Cuts: <span className="font-semibold">{splitCount} × 48&quot; {splitDirection}</span></span>
              </>
            )}
          </div>
        )}
        ...
        <div className="flex gap-4">
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            ...
            <div
              ...
              style={{
                backgroundImage: "linear-gradient(to right, rgba(100,100,120,0.10) 1px, transparent 1px), linear-gradient(to bottom, rgba(100,100,120,0.10) 1px, transparent 1px)",
            ...
            >
              <div className="absolute bottom-4 left-4 z-10 rounded-md border border-zinc-200 bg-white/95 px-3 py-1 text-xs font-medium text-zinc-600 shadow-sm">
                Reflective vinyl graphic — single-sided print
              </div>
              {isValid && (
              ...
              <div className="relative flex h-full items-center justify-center px-8 py-16">
                {isValid ? (
                  <>
                    ...
                    >
                      <div className="absolute -top-7 left-1/2 -translate-x-1/2 text-xs font-semibold text-zinc-500">{widthIn}&quot;</div>
                      <div className="absolute -bottom-7 left-1/2 -translate-x-1/2 text-xs font-semibold text-zinc-500">{widthIn}&quot;</div>
                      <div className="absolute -left-9 top-1/2 -translate-y-1/2 -rotate-90 text-xs font-semibold text-zinc-500">{heightIn}&quot;</div>
                      <div className="absolute -right-9 top-1/2 -translate-y-1/2 rotate-90 text-xs font-semibold text-zinc-500">{heightIn}&quot;</div>
                    </div>
                    ...
                    >
                      {/* Subtle reflective shimmer texture */}
                      <div
                        className="absolute inset-0 opacity-[0.08]"
                        style={{
                          backgroundImage: "repeating-linear-gradient(45deg, #fbbf24 0px, #fbbf24 2px, transparent 2px, transparent 18px)",
                        }}
                      />
                      ...
                      {panelLines.verticals.map((x, i) => {
                        ...
                        return (
                          <div
                            key={`v-${i}`}
                            className={`absolute top-0 h-full border-l-2 border-dashed ${isSel ? "border-red-500" : "border-red-400/55"}`}
                            style={{ left: `${x}%` }}
                          >
                            <div className={`absolute top-1/2 left-0 -translate-x-1/2 -translate-y-1/2 flex h-5 w-5 items-center justify-center rounded-full shadow ${isSel ? "bg-red-500" : "bg-red-400/80"}`}>
                      ...
                      {panelLines.horizontals.map((y, j) => {
                        ...
                        return (
                          <div
                            key={`h-${j}`}
                            className={`absolute left-0 w-full border-t-2 border-dashed ${isSel ? "border-red-500" : "border-red-400/55"}`}
                            style={{ top: `${y}%` }}
                          >
                            <div className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 flex h-5 w-5 items-center justify-center rounded-full shadow ${isSel ? "bg-red-500" : "bg-red-400/80"}`}>
                              <div className="h-0.5 w-3 rounded-full bg-white" />
                            </div>
                          </div>
                        );
                      ...
                      {uploadedImage ? (
                        <Image src={uploadedImage} alt="Uploaded reflective vinyl artwork preview" fill unoptimized className="object-contain" />
                      ) : (
                        <div className="absolute inset-0 flex items-center justify-center text-center text-zinc-400">
                          <div>
                            ...
                            <div className="mt-2 text-xs">{uploadingArtwork ? "Uploading artwork..." : "No artwork uploaded yet"}</div>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex h-[250px] w-full max-w-[520px] flex-col items-center justify-center rounded-[28px] border border-dashed border-zinc-300 bg-white/80 px-8 text-center shadow-inner">
                    ...
                    <div className="mt-3 text-xl font-semibold text-zinc-800">Set your dimensions to generate a live mockup</div>
                    <div className="mt-3 max-w-[30rem] text-sm leading-6 text-zinc-500">
                      Dimensions billed to the nearest whole foot (rounded up). Panel split lines shown for production guidance only — no pricing impact.
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="grid gap-2 border-t border-zinc-200 bg-zinc-50 p-3 md:grid-cols-6 xl:grid-cols-12">
              <ControlBox title="Artwork" className="md:col-span-3 xl:col-span-3" helper="JPG, PNG, PDF, AI, EPS, PSD, or SVG.">
                <div className="space-y-2">
                  <label className="inline-flex h-9 w-full cursor-pointer items-center justify-center rounded border border-zinc-300 bg-white px-3 text-sm font-medium text-zinc-700 hover:border-zinc-400">
                    <input type="file" accept="image/*,.pdf,.ai,.eps,.psd,.svg" className="hidden" onChange={onUploadArtwork} />
                  ...
                  {uploadedFileName && (
                    <div className="flex items-center justify-between gap-2 rounded border border-zinc-200 bg-white px-2 py-1.5 text-xs text-zinc-600">
                      ...
                      <button type="button" onClick={clearArtwork} className="font-semibold text-zinc-500 hover:text-zinc-900">Remove</button>
                    </div>
                  )}
              ...
              <ControlBox title="Width" className="md:col-span-2 xl:col-span-2" helper="Input value in selected unit.">
                <div className="grid grid-cols-[1fr_auto] gap-1">
                  <input type="number" min={0.1} step={0.25} value={widthStr} onChange={(e) => setWidthStr(e.target.value)} className="h-9 rounded border border-zinc-300 px-2 text-sm" />
                  <select value={widthUnit} onChange={(e) => setWidthUnit(e.target.value as DimensionUnit)} className="h-9 rounded border border-zinc-300 bg-white px-1 text-xs">
              ...
              <ControlBox title="Height" className="md:col-span-2 xl:col-span-2" helper="Input value in selected unit.">
                <div className="grid grid-cols-[1fr_auto] gap-1">
                  <input type="number" min={0.1} step={0.25} value={heightStr} onChange={(e) => setHeightStr(e.target.value)} className="h-9 rounded border border-zinc-300 px-2 text-sm" />
                  <select value={heightUnit} onChange={(e) => setHeightUnit(e.target.value as DimensionUnit)} className="h-9 rounded border border-zinc-300 bg-white px-1 text-xs">
                    <option value="inches">in</option>
                    <option value="feet">ft</option>
                  </select>
                </div>
              ...
              <ControlBox title="Contour Cut" className="md:col-span-2 xl:col-span-2" helper="+15% added to base.">
                <button type="button" onClick={() => setContourCut((v) => !v)}
                  className={`h-9 w-full rounded border px-3 text-xs font-semibold transition ${contourCut ? "border-[var(--brand-primary)] bg-[var(--brand-primary-soft)] text-[var(--brand-primary)]" : "border-zinc-300 bg-white text-zinc-700 hover:border-zinc-400"}`}>
                  {contourCut ? "Enabled" : "Disabled"}
                </button>
              </ControlBox>

              <ControlBox title="Rush" className="md:col-span-2 xl:col-span-2" helper="+100% of base price.">
                <button type="button" onClick={() => setRush((v) => !v)}
                  className={`h-9 w-full rounded border px-3 text-xs font-semibold transition ${rush ? "border-red-300 bg-red-50 text-red-700" : "border-zinc-300 bg-white text-zinc-700 hover:border-zinc-400"}`}>
                  {rush ? "Rush" : "Standard"}
                </button>
              </ControlBox>

              <ControlBox title="Split Direction" className="md:col-span-2 xl:col-span-2" helper="Visual / production only.">
                <select value={splitDirection} onChange={(e) => { setSplitDirection(e.target.value as SplitDirection); setSelectedSplit("all"); setSplitOffsets({}); }}
              ...
              <ControlBox title="Split Selected" className="md:col-span-2 xl:col-span-2" helper="Pick a split to adjust.">
                <select value={selectedSplit === "all" ? "all" : String(selectedSplit)}
                  onChange={(e) => setSelectedSplit(e.target.value === "all" ? "all" : Number(e.target.value))}
                  className="h-9 w-full rounded border border-zinc-300 bg-white px-2 text-sm">
                  <option value="all">All Splits</option>
                  {Array.from({ length: splitCount }, (_, i) => i + 1).map((n) => (
                    <option key={n} value={n}>Split {n}</option>
                  ))}
                </select>
              </ControlBox>
              ...
              <ControlBox title="Position" className="md:col-span-2 xl:col-span-3" helper="± 0.25in fine-tune. No pricing impact.">
                <div className="flex h-9 items-center gap-1">
                  <button type="button" onClick={() => adjustSplitPosition(-0.25)} disabled={splitCount === 0}
              ...
              <ControlBox title="Qty / Add" className="md:col-span-3 xl:col-span-5">
                <div className="grid grid-cols-[68px_1fr] gap-1">
                  <input type="number" min={1} value={safeQuantity}
                    onChange={(e) => setQuantity(Math.max(1, Math.floor(Number(e.target.value) || 1)))}
                    className="h-9 rounded border border-zinc-300 px-2 text-sm" />
                  <Button className="h-9 rounded bg-[var(--brand-primary)] text-xs font-semibold text-white hover:bg-[var(--brand-primary-hover)]" disabled={!isValid} onClick={addToCart}>
                    {added ? "Added" : "Add"}
                  </Button>
                </div>
              </ControlBox>
      ...
      <div className="flex w-[400px] flex-shrink-0 flex-col border-l border-zinc-200 bg-white">
        <div className="border-b border-zinc-100 px-5 py-4">
          ...
          <div className="mt-1 text-lg font-bold tracking-tight text-zinc-900">Reflective Vinyl</div>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          <aside className="space-y-4">
            <PanelCard eyebrow="Pricing" title="Reflective Vinyl Breakdown">
              <div className="space-y-1">
                <BreakdownRow label="Input size" value={pricing ? `${widthIn}" × ${heightIn}"` : "--"} muted={!pricing} />
                <BreakdownRow label="Billable size" value={pricing ? `${pricing.widthFt} ft × ${pricing.heightFt} ft` : "--"} muted={!pricing} />
                <BreakdownRow label="Total area" value={pricing ? `${pricing.sqFt} sq ft` : "--"} muted={!pricing} />
                <BreakdownRow label="Base rate" value={`${formatCurrency(REFLECTIVE_VINYL_RATE)} / sq ft`} muted={!pricing} />
                <BreakdownRow label="Base price" value={pricing ? formatCurrency(pricing.base) : formatCurrency(0)} muted={!pricing} />
                <BreakdownRow label="Contour cut (+15%)" value={pricing ? formatCharge(pricing.contourCutCharge) : formatCurrency(0)} muted={!pricing || !contourCut} />
                <BreakdownRow label="Rush (+100%)" value={pricing ? formatCharge(pricing.rushCharge) : formatCurrency(0)} muted={!pricing || !rush} />
                <BreakdownRow label="Minimum floor" value={formatCurrency(REFLECTIVE_VINYL_MIN)} muted={!pricing} />
                <div className="my-2 border-t border-zinc-200" />
                <BreakdownRow label="Unit price" value={pricing ? formatCurrency(pricing.unitPrice) : formatCurrency(0)} strong />
            ...
            <PanelCard eyebrow="Configuration" title="Order Settings">
              <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1">
                <SummaryItem label="Billable Size" value={pricing ? `${pricing.widthFt} ft × ${pricing.heightFt} ft` : "--"} />
                <SummaryItem label="Total Sq Ft" value={pricing ? `${pricing.sqFt} sq ft` : "--"} />
                <SummaryItem label="Contour Cut" value={contourCut ? "Enabled (+15%)" : "Disabled"} />
                <SummaryItem label="Rush" value={rush ? "Enabled (+100%)" : "Standard"} />
                <SummaryItem label="Split Direction" value={splitDirection.charAt(0).toUpperCase() + splitDirection.slice(1)} />
                <SummaryItem label="Split Count" value={splitCount > 0 ? `${splitCount} cut${splitCount !== 1 ? "s" : ""}` : "None"} />
              </div>
            </PanelCard>
            ...
            <PanelCard eyebrow="Pricing Note" title="Rounding Policy">
              <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3 text-sm leading-6 text-zinc-600">
                Each dimension is rounded <strong>up</strong> to the nearest whole foot before the area is computed.
                Example: 100&quot; × 100&quot; bills as 9 ft × 9 ft = 81 sq ft.
              </div>
            </PanelCard>
