...
const SHIPPING_TABLES = [
  {
    headers: ["per 3 sheets", "58+ sheets"],
    row: {
      label: '24"x36" and under',
      values: [10, 199],
      freightIndex: 1,
    },
  },
  {
    headers: ["per 3 sheets", "22+ sheets"],
    row: {
      label: '24"x36" - 32"x48"',
      values: [15, 199],
      freightIndex: 1,
...
export default function RigidSignsPricingModal({
  isOpen,
  onClose,
  pricingColumns,
  pricingRows,
          ...
          <div className="space-y-3 py-1 text-[11px] text-zinc-700">
            {SHIPPING_TABLES.map((table, tableIndex) => (
              <table key={tableIndex} className="w-full border-collapse">
                <thead>
                  <tr className="text-zinc-600">
                    ...
                    {table.headers.map((header) => (
                      <th key={`${tableIndex}-${header}`} className="pb-1 text-left font-semibold">
                        {header}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  <tr>
                    <td className="py-0.5">{table.row.label}</td>
                    {table.row.values.map((value, valueIndex) => (
                      <td key={`${tableIndex}-${valueIndex}`} className="py-0.5">
                        {formatPrice(value * markup)}
                        {valueIndex === table.row.freightIndex ? " (freight)" : ""}
                      </td>
                    ))}
                  </tr>
                </tbody>
            ...
            <div className="rounded border border-zinc-200 bg-zinc-50 px-3 py-2 text-[10px] text-zinc-600">
              All shipping prices above are displayed with a 50% markup (supplier rate x {markup.toFixed(2)}).
            </div>
          </div>
        )}
