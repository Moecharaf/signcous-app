  ...
  function addToCart() {
    ...
    cart.addItem({
      ...
      customOptions: {
        ...
        custom_roll_width_limit: `${GF2030_MAX_ROLL_WIDTH}"`,
    ...
    },
    {
      ...
      content: (
        <>
          <select
            value={laminate}
            onChange={(event) => setLaminate(event.target.value as GF2030Laminate)}
            className="h-9 w-full rounded border border-zinc-300 bg-white px-2 text-sm"
          >
            {GF2030_LAMINATE_OPTIONS.map((option) => (
      ...
      content: (
        <>
          <select
            ...
            onChange={(event) => setSplitDirection(event.target.value as GF2030SplitDirection)}
            className="h-9 w-full rounded border border-zinc-300 bg-white px-2 text-sm"
          >
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <div className="text-[11px] leading-4 text-zinc-500">Contour +15%, Rush +100%.</div>
        </>
      ),
    },
    {
          ...
          <div className="grid items-end gap-4 lg:grid-cols-1">
            <div>
              <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-2 text-xs text-zinc-500">
                ...
                <span>/</span>
                <span className="font-semibold text-zinc-900">GF 2030APAE</span>
              </nav>
              <div className="mt-2 flex flex-wrap gap-2">
                {[
                  "3.4 mil Polymeric Calendered",
                  "Air-Egress Adhesive",
              ...
              <h1 className="mt-3 text-3xl font-semibold tracking-tight text-zinc-900">GF 2030APAE Builder</h1>
              <p className="mt-1 text-sm text-zinc-600">
                3.4 mil polymeric vinyl with air-egress adhesive, 5-year durability, and automatic panel splitting for vehicle graphics, wall graphics, window decals, and more.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            <div className="border-b border-zinc-200 px-4 py-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="text-sm font-medium text-zinc-700">
                  GF 2030APAE · {pricing ? `${formatInches(pricing.widthIn)} x ${formatInches(pricing.heightIn)}` : "Set dimensions"}
                </div>
            ...
            >
              <div className="absolute right-4 top-4 z-20 rounded-lg border border-zinc-800 bg-zinc-900/95 px-4 py-2 text-right shadow-sm backdrop-blur">
                <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-blue-400">Live Total</div>
                ...
                {pricing ? (
                  <>
                    ...
                    >
                      ...
                      {uploadedImage ? (
                        <Image src={uploadedImage} alt="Uploaded GF 2030APAE artwork preview" fill unoptimized className="object-contain" />
                      ) : (
            ...
            <BuilderBottomToolbar
              ...
              action={
                <Button
                  className="h-10 w-full rounded bg-blue-500 text-xs font-semibold text-white hover:bg-blue-400"
                  disabled={!isValid}
                  onClick={addToCart}
                >
                  {added ? "Added" : "Add"}
