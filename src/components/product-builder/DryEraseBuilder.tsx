...
import {
  DRY_ERASE_MARKUP_MULTIPLIER,
  DRY_ERASE_SUPPLIER_RATE,
  calculateDryErasePrice,
  getDryErasePanelInfo,
} from "@/lib/pricing/dry-erase";

type DimensionUnit = "inches" | "feet";
type SplitDirection = "vertical" | "horizontal";

interface DryEraseBuilderProps {
  productId?: number;
}
...
export default function DryEraseBuilder({ productId = 0 }: DryEraseBuilderProps) {
  const cart = useCart();
  ...
  const pricing = useMemo(
    () =>
      isValid
        ? calculateDryErasePrice(widthIn, heightIn, { contourCut, rush }, safeQuantity)
        : null,
    [widthIn, heightIn, contourCut, rush, safeQuantity, isValid]
  );

  const panelInfo = useMemo(
    () => (isValid ? getDryErasePanelInfo(widthIn, heightIn) : null),
  ...
  function addToCart() {
    ...
    cart.addItem({
      productId,
      productName: "Dry Erase Wall Graphics",
      width: widthIn,
      height: heightIn,
      unit: "inches",
      quantity: safeQuantity,
      material: "Dry Erase Vinyl",
      doubleSided: false,
      grommets: false,
      edgeFinish: "none",
      polePockets: false,
  ...
  // Panel lines — percentage-based so lines always divide equally
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            ...
            >
              <div className="grid gap-3 md:grid-cols-[0.85fr_1.4fr_0.85fr] md:items-start md:gap-8">
                <div>
                  <div className="text-[27px] leading-[0.98] font-medium uppercase tracking-tight text-zinc-900 md:whitespace-nowrap md:text-[36px]">DRY ERASE</div>
                  <div className="mt-1 text-[11px] text-zinc-600 md:text-[12px]">Adhesive dry erase builder</div>
                </div>
                ...
                <div className="text-center md:pt-1">
                  <div className="mx-auto w-full max-w-[360px]">
                    <table className="w-full border-collapse text-[10px] leading-5 text-zinc-600 md:text-[11px]">
                      ...
                      <tbody>
                        <tr>
                          <td className="py-0.5 text-left text-zinc-500">Dry Erase</td>
                          <td className="py-0.5 text-left font-medium text-zinc-700">{formatCurrency(DRY_ERASE_SUPPLIER_RATE * DRY_ERASE_MARKUP_MULTIPLIER)} per sq ft</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
            ...
            <AdhesivePricingModal
              ...
              config={ADHESIVE_PRICING_CONFIGS.dryErase}
            />
                  ...
                  <>
                    ...
                    >
                      ...
                      {uploadedImage ? (
                        <Image
                          src={uploadedImage}
                          alt="Uploaded dry erase artwork preview"
                          fill
                          unoptimized
